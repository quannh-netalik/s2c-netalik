...
export type Tool = 'select' | 'frame' | 'rect' | 'ellipse' | 'free-draw' | 'arrow' | 'line' | 'text' | 'eraser';
...
export const shapesSlice = createSlice({
  ...
  reducers: {
    ...
    addFreeDrawShape: (state, action: PayloadAction<Parameters<typeof makeFreeDraw>[0]>) => {
      const { points } = action.payload;
      if (!points || points.length === 0) return;
      shapesAdapter.addOne(state.shapes, makeFreeDraw(action.payload));
    },
